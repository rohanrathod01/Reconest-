...
try:
    if not firebase_admin._apps:
        cred = credentials.Certificate('reconest-42f2a-firebase-adminsdk-fbsvc-301c1851fc.json')
...
class RecommendationEngine:
    def __init__(self, data):
        ...
        self.tfidf = TfidfVectorizer(
            stop_words='english', min_df=1, max_features=1000,
            ...
            for media_type in ['movie', 'book']:
                for item in self.data[media_type]:
                    item['combined_features'] = ' '.join(
                        item['genre'] + item['moods'] + item['tags'] + [item['language']] + [str(item.get('rating', 3)) + ' star']
                    )
...
def auth_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if session['user'].get('auth_type') == 'simple':
...
def simple_login():
    ...
    if data['username'] == HARDCODED_CREDENTIALS['username'] and data['password'] == HARDCODED_CREDENTIALS['password']:
        session['user'] = {
            'uid': 'simple-auth-user',
            'username': HARDCODED_CREDENTIALS['username'],
            'preferences_set': False,
            'auth_type': 'simple'
        }
        return jsonify({'success': True, 'next_page': '/choose'})
    return jsonify({'error': 'Invalid credentials'}), 401

@app.route('/choose')
@auth_required
def choose():
    if session.get('user', {}).get('preferences_set', False):
...
def set_preferences():
    ...
    session['user'] = user
    return jsonify({'success': True, 'next_page': '/home'})
...
def get_recommendations(media_type):
    try:
        ...
        recommendations = recommendation_engine.recommend(media_type, liked_ids, mood, tags, limit)
        return jsonify({
